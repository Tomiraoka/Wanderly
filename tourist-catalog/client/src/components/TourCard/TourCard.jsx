  ...
  return (
    <div className="tour-card">
      <div className="tour-card-img-wrapper">
        ...
        >
          {isFavorite ? <FaHeart className="heart-icon active" style={{color: 'red'}} /> : <FaRegHeart className="heart-icon" />}
        </button>
      </div>
      <div className="tour-card-content">
        ...
        <div className="tour-details">
          <span><FaCalendarAlt /> {tour.duration || '7 дней'}</span>
        </div>
        <div className="tour-footer">
          <div className="tour-price">
            <span className="price-value">{tour.price?.toLocaleString('ru-RU')} ₸</span>
          </div>
          <Link to={`/tours/${tour._id}`} className="tour-btn">Подробнее</Link>
        </div>
      </div>
    </div>
  );
